...
export function calculateTrkStats(data) {
  ...
  for (let i = j; i < data.length; i++) {
    ...
    // 计算两个点之间的时间差，单位为秒
    const timeDiff =
      (new Date(point.timestamp) - new Date(previousPoint.timestamp)) / 1000;
    ...
    if (elevationDiff > 0) {
      totalAscent += elevationDiff; // 累积爬升高度
    } else {
      totalDescent += -elevationDiff; // 累积下降高度
    }
  ...
  const totalTimeEndTime =
    data.length > 0 ? data[data.length - 1].timestamp : 0; // 更新总时间结束时间
  ...
  return {
    totalDistance: totalDistance / 1000, // 总距离，转换为公里
    totalTime, // 总时间，单位为秒
    totalTimeEndTime, // 总时间结束时间
    currentAltitude, // 当前海拔高度
    totalAscent, // 总爬升高度
    totalDescent, // 总下降高度
